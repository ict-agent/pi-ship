...
export function generateRunbook(m: ShipManifest): string {
	...
	if (m.requiredEnv.length > 0) {
		L.push(
			...
			'    if [ "$CFG_SECRETS" = yes ]; then',
			'      b="$(ask_yn "Auto-load them in new shells (one line added to .bashrc/.zshrc)?" n)"',
			'      [ "$b" = y ] && CFG_SHELL_ENV=yes || CFG_SHELL_ENV=no',
			'    else',
			'      CFG_SHELL_ENV=no',
			'    fi',
			'  else',
		...
		L.push("  CFG_SECRETS=no", "  CFG_SHELL_ENV=no", "");
	}
	...
	L.push(
		...
		'  # An unresolved "ask" must become a concrete answer, or the secrets step',
		'  # silently never runs and the migrated providers end up with no key.',
		'  # Carrying the values IS the reason the bundle shipped them, so default yes;',
		'  # editing a shell profile is the intrusive part, so default no.',
		'  [ "$CFG_SECRETS" = ask ] && CFG_SECRETS=yes',
		'  [ "$CFG_SHELL_ENV" = ask ] && CFG_SHELL_ENV=no',
		'fi',
		"",
	);
	...
	L.push(
		...
		'    if [ "$DRY_RUN" = 1 ]; then',
		'      warn "node $PI_NODE_MIN+ and npm are required to install pi (dry run: not installing)."',
		...
		'if [ "$PI_OK" = no ]; then',
		'  if [ "$DRY_RUN" = 1 ]; then',
		'    # A dry run must not abort: on the hosts that need it most (node too old,',
		'    # pi absent) the remaining plan is exactly what the user came to preview.',
		'    # The shortfall is reported at the end of the run instead.',
		'    :',
		'  else',
	...
	if (m.requiredEnv.length > 0) {
		step(bump(), "secrets");
		L.push(
			"# Values already present in this environment ALWAYS win; the bundle only",
			"# fills in what is missing. The loader line uses ${VAR:-...} semantics so",
			"# a value you set elsewhere is never clobbered on a later shell start.",
			'if want secrets && [ "$CFG_SECRETS" = yes ] && [ -s "$BUNDLE_DIR/.secrets.env" ]; then',
			'  # Build the .env as "only set if unset" assignments. A plain VAR=value',
			'  # would overwrite an existing variable every time a shell loads the file.',
			'  write_env() {',
			'    local src="$1" dst="$2" name value',
			'    {',
			'      printf "%s\n" "# pi-ship - secrets migrated from the source machine."',
			'      printf "%s\n" "# Each line assigns only when the variable is unset, so an"',
			'      printf "%s\n" "# existing environment value always takes precedence."',
			'      grep -E "^[A-Za-z_][A-Za-z0-9_]*=" "$src" | while IFS= read -r line; do',
			'        name="${line%%=*}"',
			'        value="${line#*=}"',
			'        # Emit NAME=<literal dollar>{NAME:-value}. .secrets.env quotes its',
			'        # values, so drop one surrounding layer or the quotes become part of',
			'        # the value. sed does the work: awk/printf escaping here is a trap.',
			'        stripped="$(sed -E "s/^[\\\x27\\\"]//; s/[\\\x27\\\"]$//" <<< "$value")"',
			'        printf \'%s=%s{%s:-%s}\\n\' "$name" "\\$" "$name" "$stripped"',
			'      done',
			'    } > "$dst"',
			'  }',
			'  if [ ! -f "$ENV_FILE" ]; then doit write_env "$BUNDLE_DIR/.secrets.env" "$ENV_FILE"; fi',
			'  doit chmod 600 "$ENV_FILE"',
			'  ok "secrets written to .env (only names absent from this machine)"',
			"",
			'  # Also install the values where pi can find them without the bundle.',
			'  # $ENV_FILE lives in the bundle directory - a build artifact the user may',
			'  # delete or move. A provider referencing $KSYUN_API_KEY would then break',
			'  # with no obvious cause, so keep a copy in the agent dir.',
			'  AGENT_ENV="$AGENT_DIR/.env"',
			'  if [ ! -f "$AGENT_ENV" ]; then',
			'    doit write_env "$BUNDLE_DIR/.secrets.env" "$AGENT_ENV"',
			'    doit chmod 600 "$AGENT_ENV"',
			'    ok "secrets also placed at $AGENT_ENV (survives deleting the bundle)"',
			'  fi',
			"",
			'  # Optionally make new shells load them. This is a separate answer from',
			'  # writing the file: editing someone shell profile is a bigger step than',
			'  # dropping a file in their agent dir, and some people manage env vars',
			'  # themselves (direnv, a secrets manager).',
			'  if [ "$CFG_SHELL_ENV" = yes ]; then',
			'    MARK="# pi-ship secrets"',
			'    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do',
			'      [ -e "$rc" ] || continue',
			'      if grep -qF "$MARK" "$rc" 2>/dev/null; then',
			'        info "$(basename "$rc") already loads $AGENT_ENV"',
			'      else',
			'        # Build the block first, then append with a plain redirect. Passing',
			'        # the printf straight to doit echoed a multi-line command into the',
			'        # user profile, leaving stray text at the top of their .bashrc.',
			'        { printf "\\n%s\\n" "$MARK"; printf \'[ -f "%s" ] && set -a && . "%s" && set +a\\n\' "$AGENT_ENV" "$AGENT_ENV"; } >> "$rc"',
			'        ok "added a secrets loader to $(basename "$rc")"',
			'      fi',
			'    done',
			'  else',
			'    info "not touching your shell profile; load them yourself with:"',
			'    info "  set -a; . $AGENT_ENV; set +a"',
			'  fi',
			"fi",
			"",
	...
	L.push(
		...
		'  if ! have pi; then',
		'    echo',
		'    warn "pi is not available — a real run would stop after this preview."',
		'    info "install it first:  npm install -g @earendil-works/pi-coding-agent"',
		'  fi',
		'  echo',
	...
	L.push(
		'info "  pi"',
		'info "If pi was already running, use /reload instead."',
	);
	// The PATH entry this script wrote only applies to NEW shells, so `pi` is
	// still not on PATH in the shell that ran the installer. Saying "now run pi"
	// without this line produced a confusing "Command pi not found" (which on
	// Ubuntu then suggests an unrelated snap package).
	L.push(
		'if ! have pi; then',
		'  echo',
		'  warn "pi is not on PATH in THIS shell yet - the PATH entry applies to new shells."',
		'  info "Start a new shell, or run:  source ~/.bashrc  (or ~/.zshrc)"',
		'fi',
		"",
	);
