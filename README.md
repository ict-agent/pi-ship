...
- **Nothing is overwritten.** `models.json` and `settings.json` are merged;
  existing providers and keys win. Changed files are backed up to
  `*.bak-pi-migrate` first.
...
- `settings.json` `packages` is an additive merge, so the result is a union with
  whatever the target already had, not a strict snapshot. Strictness would
  require `pi uninstall`, which deletes things the user may want.
